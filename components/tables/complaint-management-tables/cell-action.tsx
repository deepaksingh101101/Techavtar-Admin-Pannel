...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const editComplaintMessage = () => {
    router.push(`/complaint-management/editComplaintMessage/${data.complaintId}`); 
  };

  const viewComplaint = () => {
    router.push(`/complaint-management/viewComplaint/${data.complaintId}`); 
  };

  const updateComplaintStatus = () => {
    router.push(`/complaint-management/updateComplaintStatus/${data.complaintId}`); 
  };
      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem onClick={() => setOpen(true)}>
            <Trash className="mr-2 h-4 w-4" /> Delete Complaint
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </>
